...
namespace JavnoNadmetanje.ServiceCalls
{
    public class ParcelaService : IParcelaService
    {
        ...
        public async Task<ParcelaDto> GetParcelaById(Guid parcelaId)
        {
            try
            {
                using var httpClient = new HttpClient();
                Uri url = new Uri($"{ configuration["Services:DokumentService"] }api/parcela/" + parcelaId);
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    return JsonConvert.DeserializeObject <ParcelaDto>(content);
                }
                return default;
            }
            catch
